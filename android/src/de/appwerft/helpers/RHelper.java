...
public class RHelper {
	...
	public static int getAppString(String str) {
		try {
			return TiRHelper.getApplicationResource("string." + str);
		} catch (ResourceNotFoundException e) {
			e.printStackTrace();
	...
	public static CharSequence getAppText(String str) {
		int id = 0;
		try {
			id = TiRHelper.getApplicationResource("string." + str);
		} catch (ResourceNotFoundException e) {
			e.printStackTrace();
		}
		Context ctx = TiApplication.getInstance().getApplicationContext();
		return ctx.getText(id);
	}

	public static CharSequence getAppText(int id) {
		Context ctx = TiApplication.getInstance().getApplicationContext();
		return ctx.getText(id);
	}
	...
	public static int getDrawable(String str) {
		...
		try {
			if (str.contains("android:")) {
				...
				Log.d("RHelper", parts.toString());
				id = TiRHelper.getAndroidResource("drawable." + str);
				Log.d("RHelper", "id=" + id);
				if (id == 0) {
					id = TiRHelper.getAndroidResource("drawable." + parts[1]);
				}
				Log.d("RHelper", "id=" + id);
				return id;
			} else
